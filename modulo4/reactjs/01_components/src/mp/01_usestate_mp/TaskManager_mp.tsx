...
export default function TaskManager_mp() {
  ...
  return (
    <div style={{ maxWidth: 380 }}>
      ...
      {tasks.length === 0 && (
        <p style={{ color: '#999', fontSize: 14 }}>No hay mantenimientos. ¡Flota lista!</p>
      )}
      ...
      <ul style={{ listStyle: 'none', padding: 0 }}>
        {tasks.map((task) => (
          ...
          >
            ...
            <span style={{ flex: 1, textDecoration: task.done ? 'line-through' : 'none', color: task.done ? '#aaa' : '#333' }}>
              {task.title}
            </span>
            <span style={{ flex: 1, textDecoration: task.done ? 'line-through' : 'none', color: task.done ? '#aaa' : '#333' }}>
              {task.text}
      ...
      {tasks.length > 0 && (
        <p style={{ fontSize: 13, color: '#888', marginTop: 8 }}>
          {tasks.filter((t) => t.done).length} de {tasks.length} mantenimientos completados
        </p>
      )}
    </div>
  )
